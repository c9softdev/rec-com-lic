...
import {
  ...
  MultiSelectOption,
} from '../multi-select-dropdown/multi-select-dropdown.component';
...
import {
  ...
  GlobalSettings,
} from '../../core/services/global-settings.service';
...
export class JobseekerManagerComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  readonly ALWAYS_ACTIVE_IDS = new Set<string>([
    'quickPassport',
    'fsssubCat',
    'fssplCat',
    'sfdate',
  ...
  ngAfterViewInit(): void {
    try {
      if (this.remarkModalEl?.nativeElement) {
        this.remarkModalInstance = new bootstrap.Modal(
          this.remarkModalEl.nativeElement,
          {
            backdrop: 'static',
            keyboard: false,
          }
      ...
      if (this.resumeLogModalEl?.nativeElement) {
        this.resumeLogModalInstance = new bootstrap.Modal(
          this.resumeLogModalEl.nativeElement,
          {
            backdrop: 'static',
            keyboard: false,
          }
  ...
  loadParentCategories(): void {
    this.commonService.loadParentCategories().subscribe((res) => {
      if (res?.status === 'success' && res.data) {
        this.parentCategories = Object.entries(res.data).map(([id, name]) => ({
          id,
          name,
        }));
      }
  ...
  onParentCategoryChange(): void {
    ...
    if (searchCatId) {
      this.commonService.loadSubCategories(searchCatId).subscribe((res) => {
        if (res?.status === 'success' && res.data) {
          this.subCategories = Object.entries(res.data).map(([id, name]) => ({
            id,
            name,
          }));
          this.subCategoryOptions = this.subCategories.map((s: any) => ({
            id: String(s.id),
            name: String(s.name),
          }));
        }
  ...
  onAddFormSubCategoryChange(): void {
    ...
    if (catId && subCatId) {
      this.commonService
        .loadSpecializations(catId, subCatId)
        .subscribe((res) => {
          if (res?.status === 'success' && res.data) {
            this.specializations = Object.entries(res.data).map(
              ([id, name]) => ({ id, name })
            );
          }
        });
    ...
    if (catId && Array.isArray(subCats) && subCats.length > 0) {
      ...
      subCats.forEach((subCatId: string) => {
        this.commonService
          ...
          .subscribe((res) => {
            if (res?.status === 'success' && res.data) {
              const items = Object.entries(res.data).map(([id, name]) => ({
                id,
                name,
              }));
              // merge unique
              ...
              this.specializationOptions = this.specializations.map(
                (s: any) => ({ id: String(s.id), name: String(s.name) })
              );
            }
          });
      });
  ...
  loadRemarks(): void {
    ...
    this.commonService.post(payload).subscribe((res) => {
      if (res?.status === 'success' && res.data?.list) {
        this.remarks = Object.entries(res.data.list).map(([id, name]) => ({
          id,
          name,
        }));
      }
  ...
  loadCountries(): void {
    this.commonService.loadCountries().subscribe(
      (res: any) => {
        const list =
          res?.status === 'success' ? res.data?.list : res?.list || null;
        if (Array.isArray(list)) {
          this.countries = list;
        } else if (list && typeof list === 'object') {
          this.countries = Object.entries(list).map(([id, nameOrObj]: any) => {
            if (typeof nameOrObj === 'string' || typeof nameOrObj === 'number')
              return { id, name: nameOrObj };
  ...
  loadQualificationLists(): void {
    // Graduation
    this.commonService
      .loadQualifications({ parentCategory: '6' })
      .subscribe((res: any) => {
        if (res?.status === 'success' && res.data?.list) {
    ...
    // Post Graduation
    this.commonService
      .loadQualifications({ parentCategory: '7' })
      .subscribe((res: any) => {
        if (res?.status === 'success' && res.data?.list) {
    ...
    // Fellowship/PhD
    this.commonService
      .loadQualifications({ parentCategory: '41' })
      .subscribe((res: any) => {
        if (res?.status === 'success' && res.data?.list) {
  ...
  toggleSection(section: 'personal' | 'passport' | 'education'): void {
    ...
    if (section === 'passport')
      this.showPassportSection = !this.showPassportSection;
  ...
  downloadExistingResume(): void {
    if (this.existingResumeUrl) {
      ...
      this.downloadService.downloadFileFromApi(
        this.commonService.postBlob(payload),
        filename
      );
    }
  }
      ...
      this.commonService.post(payload).subscribe({
        next: (res: any) => {
          if (res?.status === 'success') {
            ...
            this.sweetAlert.showToast(
              res?.message || 'Failed to delete resume file.',
              'error'
            );
          }
        },
      ...
      this.commonService.post(payload).subscribe({
        next: (res: any) => {
          if (res?.status === 'success') {
            ...
            this.sweetAlert.showToast(
              res?.message || 'Failed to delete document file.',
              'error'
            );
          }
        },
  ...
  onSubmitJobseeker(): void {
    ...
    if (formValue.date_issue)
      formValue.date_issue = normalizeToApiDate(formValue.date_issue);
    if (formValue.date_exp)
      formValue.date_exp = normalizeToApiDate(formValue.date_exp);
          ...
          if (res?.status === 'success') {
            ...
            if (
              this.notifyOpenerAndClose(
                'saved',
                this.editJobseekerId || (res?.data && res.data.id) || ''
              )
            )
              return;
            // If user only has "Add Jobseeker" permission, show dialog for next action
            if (this.hasOnlyAddJobseekerPermission()) {
              this.showPostSaveDialog();
            } else {
              // Otherwise navigate back to jobseeker manager list
              this.router.navigate(['/jobseeker-manager']);
            }
          } else {
            this.sweetAlert.showError(
              res?.message || 'Failed to save jobseeker.'
            );
          }
        },
        () => {
          this.sweetAlert.showError(
            'Failed to save jobseeker. Please try again.'
          );
        }
      );
    } else {
  ...
  listJobseekers(): void {
    ...
    formValue.fsssubCat = Array.isArray(formValue.fsssubCat)
      ? formValue.fsssubCat.map((v: any) => Number(v))
      : [];
    formValue.fssplCat = Array.isArray(formValue.fssplCat)
      ? formValue.fssplCat.map((v: any) => Number(v))
      : [];
    formValue.sddcity = Array.isArray(formValue.sddcity)
      ? formValue.sddcity.map((v: any) => Number(v))
      : [];
    ...
    this.commonService.post(payload).subscribe(
      (res) => {
        ...
        if (res?.status === 'success' && res.data?.list) {
          ...
          this.totalPages = Math.ceil(
            Number(this.totalRecords) / this.pageSize
          );
          this.noDataMessage = '';
      ...
      () => {
        this.sweetAlert.showError(
          'Failed to load jobseekers. Please try again.'
        );
        this.jobseekers = [];
        this.totalRecords = '0';
        this.totalPages = 0;
  ...
  onExport(): void {
    ...
    if (this.isInCooldown) {
      const seconds = Math.ceil(
        ((this.exportCooldownUntil || 0) - Date.now()) / 1000
      );
      this.sweetAlert.showToast(
        `Please wait ${seconds}s before exporting again.`,
        'info'
      );
      return;
    }
    ...
    if (limit > 0 && total > limit) {
      this.sweetAlert.showToast(
        `Export restricted to ${limit} records. Please narrow your search.`,
        'warning'
      );
      return;
    }
    ...
    this.commonService.post(payload).subscribe({
      next: (res) => {
        ...
        if (res?.status === 'success' && res.data?.content) {
          ...
          this.sweetAlert.showToast(
            'Jobseekers exported successfully.',
            'success'
          );
          this.exportCooldownUntil = Date.now() + 10000; // 10s cooldown
        } else {
          this.sweetAlert.showToast(
            res?.message || 'No jobseekers found.',
            'info'
          );
        }
      },
      error: () => {
        ...
        this.sweetAlert.showError(
          'Failed to export jobseekers. Please try again.'
        );
      },
      complete: () => {
  ...
  onSearch(): void {
    const hasAnyCriteria = Object.values(this.searchForm.value).some(
      (v: any) => {
    ...
    if (!hasAnyCriteria)
      this.sweetAlert.showToast(
        'Please enter/select at least one search criteria',
        'warning'
      );
    this.currentPage = 1;
  ...
  onChangeVisibilityClick(): void {
    if (this.selectedItems.size === 0) {
      this.sweetAlert.showToast(
        'Please select at least one item to change visibility',
        'warning'
      );
      return;
    }
    this.sweetAlert
      .confirmStatusChange(
        'Are you sure you want to change the visibility of selected jobseekers?'
      )
      .then((result: any) => {
  ...
  private confirmChangeVisibility(): void {
    ...
    this.commonService.post(payload).subscribe(
      (res) => {
        ...
        if (res?.status === 'success') {
          this.sweetAlert.showToast(
            res.message || 'Visibility updated.',
            'success'
          );
          this.selectedItems.clear();
          this.listJobseekers();
          ...
          this.sweetAlert.showError(
            res?.message || 'Failed to update visibility'
          );
        }
      },
      () => {
        this.loadingService.hide();
        this.sweetAlert.showError(
          'Failed to change visibility. Please try again.'
        );
      }
    );
  ...
  onDeleteClick(): void {
    ...
    this.sweetAlert
      .confirmDelete('Are you sure you want to delete selected jobseekers?')
      .then((result: any) => {
        if (result.isConfirmed) {
          this.confirmDelete();
        }
  ...
  private confirmDelete(): void {
    ...
    this.commonService.post(payload).subscribe(
      (res) => {
        ...
        if (res?.status === 'success') {
          this.sweetAlert.showToast(
            res.message || 'Jobseekers deleted.',
            'success'
          );
          this.selectedItems.clear();
          this.listJobseekers();
      ...
      () => {
        this.loadingService.hide();
        this.sweetAlert.showError(
          'Failed to delete jobseekers. Please try again.'
        );
      }
    );
  ...
  onShowAddForm(): void {
    // Check if user has permission to add jobseeker (menuId 29: Add Jobseeker/CV)
    if (!this.sessionService.hasMenu('29')) {
      this.sweetAlert.showError(
        'You do not have permission to add a jobseeker.'
      );
      return;
    }
    this.listRecdFlg = false;
  ...
  /**
   * Check if user has only "Add Jobseeker" (menuId 29) permission but NOT "JobSeeker Manager" (menuId 24)
   */
  private hasOnlyAddJobseekerPermission(): boolean {
    return (
      this.sessionService.hasMenu('29') && !this.sessionService.hasMenu('24')
    );
  }

  /**
   * Show dialog after successful save asking user to keep adding CV or go to dashboard
   * This is only shown for users with only "Add Jobseeker" permission
   */
  private showPostSaveDialog(): void {
    Swal.fire({
      title: 'CV Saved Successfully!',
      text: 'What would you like to do next?',
      icon: 'success',
      showCancelButton: true,
      confirmButtonText: 'Keep Adding CV',
      ...
      if (result.isConfirmed) {
        // Keep adding CV - reset form and stay on add page
        this.jobseekerForm.reset();
        this.selectedResumeFile = null;
        this.selectedDocumentFile = null;
        this.existingResumeUrl = '';
  ...
  onCancelClick(): void {
    ...
    if (this.notifyOpenerAndClose('cancelled', this.editJobseekerId || ''))
      return;

    // If user only has "Add Jobseeker" permission (not JobSeeker Manager), route to dashboard on cancel
    if (this.hasOnlyAddJobseekerPermission()) {
      this.router.navigate(['/dashboard']);
    } else {
      // Otherwise navigate back to jobseeker manager list
  ...
  onSendEmail(): void {
    ...
    if (this.selectedItems.size === 0) {
      this.sweetAlert.showAlertCenter(
        'No Jobseekers Selected',
        'Please select at least one jobseeker to send email.',
        'warning'
      );
      return;
    }
      ...
      if (typeof window !== 'undefined' && (window.opener as any)) {
        try {
          window.opener.postMessage(
            { type: 'jobseeker-updated', id: id || '', action },
        ...
        if (res?.status === 'success' && res.data) {
            ...
            this.commonService.loadSubCategories(catIdStr).subscribe({
              next: (subRes: any) => {
                if (subRes?.status === 'success' && subRes.data) {
                  this.subCategories = Object.entries(subRes.data).map(
                    ([id, name]) => ({ id, name })
                  );
                  ...
                  this.commonService
                    ...
                    .subscribe({
                      next: (specRes: any) => {
                        ...
                        const selectedSpec = row.int_subspeci_id
                          ? String(row.int_subspeci_id)
                          : '';
                        this.jobseekerForm.patchValue({
                          ssplCat: selectedSpec,
                        });
          ...
          const applyCityPatch = () => {
            ...
            const loc = (this.locations || []).find(
              (l: any) =>
                String(l.country_id || l.CountryId || l.countryid || '') ===
                  countryIdStr ||
                String(l.country || l.Country || '') === countryIdStr ||
                String(l.countryid || '') === countryIdStr
            );
            // prefer int_state or var_city from row
            this.jobseekerForm.patchValue({
              ddcity: row.int_state
                ? String(row.int_state)
                : row.var_city || (loc ? String(loc.id) : ''),
            });
          ...
          this.sweetAlert.showError(
            res?.message || 'Failed to load jobseeker details.'
          );
        }
      },
      error: () => {
        this.loadingService.hide();
        this.sweetAlert.showError(
          'Failed to load jobseeker details. Please try again.'
        );
      },
    });
    // this.updateFloatingLabels();
  ...
  onViewClick(item: any): void {
    ...
    if (this.comp_id !== this.superAdminID && this.viewresumechk !== '1') {
      this.sweetAlert.showToast(
        'You do not have permission to view resumes.',
        'error'
      );
      return;
    }
    ...
    const absolute = `${baseHref}print-resume/${encodeURIComponent(
      id
    )}?mode=view`;
  ...
  onPrintClick(item: any): void {
    ...
    if (this.comp_id !== this.superAdminID && this.printresumechk !== '1') {
      this.sweetAlert.showToast(
        'You do not have permission to print resumes.',
        'error'
      );
      return;
    }
  ...
  onJobseekerEmailBlur(): void {
    ...
    const payload: any = {
      ...
      InputData: [
        this.isEditMode ? { email, id: this.editJobseekerId || '' } : { email },
      ],
    };
    ...
    this.commonService.post(payload).subscribe(
      (res) => {
        ...
        const duplicate =
          res?.status === 'success' && msg.toLowerCase().includes('exists');
        this.setEmailDuplicate(duplicate);
        this.emailExistsMessage = duplicate
  ...
  get isInCooldown(): boolean {
    return (
      this.exportCooldownUntil !== null &&
      Date.now() < (this.exportCooldownUntil as number)
    );
  }
  ...
  get locationsList(): any[] {
    ...
    if (l && typeof l === 'object') {
      return Object.entries(l).map(([id, val]: any) => ({
        id,
        ...(val as any),
      }));
    }
  ...
  onAssignCvsToProject(): void {
    ...
    if (this.selectedItems.size === 0) {
      this.sweetAlert.showToast(
        'Please select at least one CV to assign.',
        'warning'
      );
      return;
    }
    ...
    const projectName =
      this.projects.find((p) => p.id === this.selectedProjectId)?.task_name ||
      'Selected Project';
  ...
  private assignCvsToProject(): void {
    ...
    this.commonService.post(payload).subscribe({
      next: (res: any) => {
        ...
        if (res?.status === 'success') {
          this.sweetAlert.showToast(
            res.message || 'CVs assigned successfully.',
            'success'
          );
          this.selectedItems.clear();
  ...
  updateFloatingLabels(): void {
    setTimeout(() => {
      ...
      inputs.forEach((input: Element) => {
        const element = input as HTMLInputElement | HTMLSelectElement;
        const isAlwaysActiveMultiSelect =
          (element as any).dataset &&
          (element as any).dataset.alwaysActive === 'true';
        if (
          this.ALWAYS_ACTIVE_IDS.has(element.id) ||
          isAlwaysActiveMultiSelect
        ) {
          element.classList.add('active');
          return;
        }
        if (
          ...
          (element instanceof HTMLSelectElement && element.selectedIndex > 0)
        ) {
          element.classList.add('active');
        } else {
          element.classList.remove('active');
